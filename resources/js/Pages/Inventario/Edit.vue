...
const form = useForm({
    ...
    lada: props.numero.lada ?? '',
    tipo: props.numero.tipo ?? 'movil',
    estado: props.numero.estado ?? 'disponible',
    plan_id: props.numero.plan_id ?? '',
});

const submit = () => form.put(route('inventario.update', props.numero.id));
</script>
...
<template>
    ...
    <AuthenticatedLayout>
        <template #header>
            <h2 class="text-xl font-bold tracking-tight text-slate-800">Editar número</h2>
        </template>
            ...
            <form @submit.prevent="submit" class="space-y-5 rounded-2xl border border-slate-200 bg-white p-8 shadow-sm">
                ...
                <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
                    <div>
                        ...
                        <TextInput id="lada" v-model="form.lada" class="mt-1 block w-full" />
                    </div>
                    <div>
